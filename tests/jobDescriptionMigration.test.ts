...
const recursionFix = readFileSync(
  new URL('../supabase/migrations/20260822114500_fix_job_description_ack_rls_recursion.sql', import.meta.url),
  'utf8',
);
...
assert.match(migration, /where id = new\.job_description_id and active/, 'the signing trigger enforces the current published definition');
assert.doesNotMatch(recursionFix, /from public\.job_descriptions/, 'acknowledgement INSERT RLS does not recurse through the historical-definition SELECT policy');
