...
export default function PersonnelFile({ applicants, staff, documents, templates, onNavigate }: {
  ...
  return <div className="space-y-5" id="shc-personnel-file">
    <div><h2 className="text-xl font-bold text-slate-900">Personnel Files</h2><p className="text-xs font-medium text-slate-500">A live control view derived from authoritative recruitment, compliance, HR and document records.</p></div>
    <div className="grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 space-y-5">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-center gap-3"><div className="rounded-xl bg-purple-50 p-3 text-purple-800"><UserRound className="h-5 w-5" /></div><div><h3 className="font-bold text-slate-900">{selected?.name}</h3><p className="text-xs text-slate-500">{selected?.roleName || 'Role not assigned'} - {selected?.lifecycle} - Account {selected?.accountStatus}</p></div></div><div className="rounded-xl bg-slate-50 px-4 py-3 text-right"><div className="text-2xl font-black text-slate-900">{summary.percentage}%</div><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Required file controls</div></div></div>
        ...
        {loading ? <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-xs text-slate-500">Loading authoritative personnel records...</div> : categories.map(category => {
          ...
          return <section key={category} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-5 py-4"><div><h3 className="text-sm font-bold text-slate-900">{category}</h3><p className="text-[10px] text-slate-500">{categorySummary?.required ? `${categorySummary.complete} of ${categorySummary.required} applicable required controls complete` : 'No currently applicable required controls'}</p></div><FileCheck2 className="h-5 w-5 text-slate-400" /></div>
            <div className="divide-y divide-slate-100">{categoryItems.map(entry => <div key={entry.key} className="p-4 sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h4 className="text-xs font-bold text-slate-900">{entry.displayName}</h4>{entry.blocking && <span className="text-[9px] font-bold uppercase text-rose-600">Required</span>}</div><p className="mt-1 text-xs leading-5 text-slate-600">{entry.reason}</p><p className="mt-2 text-[10px] text-slate-400">Source: {entry.source.workflow}{entry.source.timestamp ? ` - ${formatDate(entry.source.timestamp)}` : ''} - {entry.derivation}</p><p className="mt-1 text-[10px] text-slate-400">Next action: {entry.responsibleParty === 'applicant' ? 'Applicant / employee' : 'SHC office'}{entry.source.evidenceId ? ' - controlled evidence linked' : ''}</p></div><div className="flex shrink-0 items-center gap-2"><span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold ${statusClass[entry.status]}`}>{entry.status}</span>{entry.derivation !== 'future-workflow' && <button type="button" onClick={() => onNavigate(entry.source.route, selected!)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1 text-[10px] font-bold text-slate-700 hover:bg-slate-50">Open <ArrowRight className="h-3 w-3" /></button>}</div></div></div>)}</div>
          </section>;
        })}
