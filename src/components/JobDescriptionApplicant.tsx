...
import { acknowledgementTextFor, currentJobDescriptionComplete, jobDescriptionStatus } from '../lib/jobDescriptions';
...
export default function JobDescriptionApplicant({ applicant, authenticatedUserId, role, onStateChange }: Props) {
  ...
  const acknowledgementText = acknowledgementTextFor(current);
  ...
  return <div className="space-y-4">
    ...
    {acknowledgements.filter(item => item.jobDescriptionId !== current.id).length > 0 && <section className="rounded-xl border bg-white p-4"><h3 className="flex items-center gap-1 text-[10px] font-black uppercase text-slate-500"><Clock className="h-3.5 w-3.5" /> Previous signed Job Descriptions</h3><div className="mt-2 space-y-2">{acknowledgements.filter(item => item.jobDescriptionId !== current.id).map(item => <details key={item.id} className="rounded-lg bg-slate-50 p-3 text-[10px]"><summary className="flex cursor-pointer list-none flex-wrap justify-between gap-2"><span className="font-bold">{item.roleName} · v{item.jdVersion}</span><span><CheckCircle2 className="mr-1 inline h-3 w-3 text-emerald-600" />{new Date(item.signedAt).toLocaleDateString()}</span></summary><div className="mt-3"><JobDescriptionDocument title={item.jdTitle} roleName={item.roleName} version={item.jdVersion} effectiveDate={item.jdEffectiveDate} content={item.contentSnapshot} acknowledgement={{ text: item.acknowledgementText, signerName: item.signerName, signatureType: item.signatureType, signatureValue: item.signatureValue, signedAt: item.signedAt }} /></div></details>)}</div></section>}
  </div>;
}
